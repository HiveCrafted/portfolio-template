...
The following steps demonstrate how to use portfolio-template as **GitHub Pages remote theme**.

1. [Download][download] portfolio-template as .zip from official [repo][repo] then extract it.
1. Rename **`portfolio-template-master/`** to **`<your-username>.github.io/`**
...
1. Finally, test portfolio-template [locally][locally] then [publish][publish] it to [GitHub Pages][gh-pages].
1. _[Optional]_ To use a specific [version][versions] of portfolio-template _(defaults to latest version)_:

    ```yml
    remote_theme: HiveCrafted/portfolio-template@v1.0.0
    ```
